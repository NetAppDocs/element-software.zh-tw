...
* 使用更新mNode中的儲存叢集管理認證 link:../api/reference_element_api_modifyclusteradmin.html["修改叢集管理API"]。
...
* 對於H410S和H610S節點、 link:task_post_deploy_credential_change_ipmi_password.html["變更預設的ipm密碼"]。
...
* link:task_system_manage_mfa_set_up_multi_factor_authentication.html["啟用多因素驗證"]
