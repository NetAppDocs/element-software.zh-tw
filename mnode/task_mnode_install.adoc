...
在遵循此程序之前，您應該先瞭解link:../concepts/concept_solidfire_concepts_volumes.html#persistent-volumes["持續磁碟區"]及是否要使用它們。持續磁碟區為選用功能、但建議在虛擬機器（VM）遺失時、進行管理節點組態資料恢復。
...
.步驟
. 從 NetApp 支援網站的頁面下載 OVA 或 ISO 以供安裝link:https://mysupport.netapp.com/site/products/all/details/element-software/downloads-tab["Element軟體"^]。新的 OVA 包含更新的 SSL 憑證。
+
.. 選擇*下載最新版本*並接受EULA。
.. 選取您要下載的管理節點映像。
