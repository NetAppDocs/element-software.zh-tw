...
summary: 您可以使用SetNodeSSLCertificate方法來設定管理節點的使用者SSL憑證和私密金鑰。 
---
...
[role="lead"]
您可以使用「etNodeSSLCertificate」方法來設定管理節點的使用者SSL憑證和私密金鑰。
...
憑證
 a| 
憑證的PEP-編碼文字版本。*注意：*設定節點或叢集憑證時、憑證必須包含serverauth的extendedKeyusage副檔名。此擴充功能可讓您在一般作業系統和瀏覽器上使用憑證、而不會發生錯誤。如果沒有副檔名、API將拒絕該憑證為無效。
 a| 
字串
 a| 
無
