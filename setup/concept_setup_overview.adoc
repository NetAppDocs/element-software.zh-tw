...
. 選取下列其中一項：
+
** link:../setup/concept_setup_configure_a_storage_node.html["使用儲存節點設定叢集"]
+
您可以在機架單元中安裝及連接節點並開啟電源之後、使用Element軟體來設定具有儲存節點的叢集、並加以管理。然後、您可以在儲存系統中安裝及設定其他元件。

** link:../setup/concept_setup_fc_configure_a_fibre_channel_node.html["使用Fibre Channel節點設定叢集"]
+
